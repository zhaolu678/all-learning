...
public class SinaCollection {

    /**
     * 获取新浪微博热门话题，热搜榜，获取10条。
     * https://s.weibo.com/top/summary?cate=realtimehot
     * @return
     */
    public static List<News> getSinaNews() {
        List<News> list = new ArrayList<>();
        String sturl = "https://s.weibo.com/ajax/jsonp/gettopsug?uid=&ref=PC_topsug&url=https://s.weibo.com/top/summary?cate=realtimehot";
        String result = HttpUtil.HttpGet(sturl);
        result = result.substring(result.indexOf("{\"data\":{"), result.indexOf("}]}}") + 4);
        JSONObject hostObject = JSONObject.parseObject(result);
        JSONObject dataObject = (JSONObject) hostObject.get("data");
        JSONArray jsonArray = (JSONArray) dataObject.get("list");

        for (int i = 0; i < jsonArray.size(); i++) {
            News news = new News();
            JSONObject newsObject =  jsonArray.getJSONObject(i);
            news.setBurst_time(newsObject.get("burst_time")+"");
            news.setEmoticon(newsObject.get("emoticon")+"");
            news.setName(newsObject.get("note")+" "+newsObject.get("emoticon"));
            news.setWord(newsObject.get("word")+"");
            news.setFlag(newsObject.get("flag")+"");
            news.setNum(newsObject.get("num")+"");
            news.setLink("https://s.weibo.com/weibo?q=%23"+newsObject.get("note")+"%23&Refer=top");
            list.add(news);
        }
        return list;
    }
...
//        JSONObject hostObject = JSONObject.parseObject(result);
//        JSONObject dataObject = hostObject.getJSONObject("data");
//        JSONArray jsonArray = dataObject.getJSONArray("list");
//        //System.out.println(jsonArray);
//        for (int i = 0; i < jsonArray.size(); i++) {
//            JSONObject newsObject =  jsonArray.getJSONObject(i);
//            System.out.println(newsObject.get("note"));
//            System.out.println(newsObject.get("word"));
//            System.out.println(newsObject.get("burst_time"));
//            System.out.println(newsObject.get("num"));
//            System.out.println(newsObject.get("flag"));
//            System.out.println(newsObject.get("emoticon"));
//        }
//    }
